...
with open(genecounts) as tsvfile:  
    ...
    # uncomment this line below for genes found in 15% - 95% of all genomes
    max_species_number = total_species * 0.92
    
    for og in reader:
        current_og = og[1:-1]
        current_og_ints = [int(x) for x in current_og]
        # uncomment line below if only min_species_proportion is used
        #if max_copies == 1 and max(current_og_ints) == 1 and (total_species - current_og_ints.count(0)) >= min_species_number:
        if max_copies == 1 and max(current_og_ints) == 1 and (total_species - current_og_ints.count(0)) >= min_species_number and (total_species - current_og_ints.count(0)) < max_species_number:
        ...
        if max(current_og_ints) <= max_copies and (total_species - current_og_ints.count(0)) >= min_species_number and (total_species - current_og_ints.count(0)) < max_species_number:
